...
{
    auto lexer = Lexer{"foreach include include_rules ifdef ifndef ifeq ifneq else endif export import preload error run .gitignore"};
    ...
    REQUIRE(lexer.next().is(TokenType::KwPreload));
    REQUIRE(lexer.next().is(TokenType::KwError));
    REQUIRE(lexer.next().is(TokenType::KwRun));
