...
SCENARIO("A group member whose name carries a percent flag reaches its consumer whole", "[e2e][build][groups]")
{
    GIVEN("a group whose member file name contains a percent flag")
    {
        auto f = E2EFixture { "glob_mixed_space" };
        f.write_file("a%1fb.c", "x\n");
        f.write_file("dummy.txt", "y\n");
        f.write_file(
            "Tupfile",
            ": a%1fb.c |> touch %o |> %B.h <gen>\n"
            ": dummy.txt | <gen> |> echo START %<gen> END > %o |> out.txt\n"
        );
        f.mkdir("build");
        REQUIRE(f.pup({ "configure", "-B", "build" }).success());

        WHEN("a consumer of the group is built")
        {
            REQUIRE(f.build({ "-B", "build" }).success());

            THEN("the consumer names the member that exists, not a re-expansion of it")
            {
                REQUIRE(f.exists("build/a%1fb.h"));
                INFO("out.txt: " << f.read_file("build/out.txt"));
                REQUIRE(f.read_file("build/out.txt") == "START build/a%1fb.h END\n");
            }
        }
    }
}

SCENARIO("A percent escape in a group rule is not expanded a second time", "[e2e][build][groups]")
{
    GIVEN("a rule that spells a literal percent and also references a group")
    {
        auto f = E2EFixture { "glob_mixed_space" };
        f.write_file("dummy.txt", "y\n");
        f.write_file(
            "Tupfile",
            ": |> echo gen > %o |> gen.h <gen>\n"
            ": dummy.txt | <gen> |> echo 'F=%%f G=%%<gen>' %<gen> > %o |> out.txt\n"
        );
        f.mkdir("build");
        REQUIRE(f.pup({ "configure", "-B", "build" }).success());
        ...
        WHEN("the rule runs")
        {
            ...
            THEN("each escaped percent reaches the shell as one percent, introducing nothing")
            {
                INFO("out.txt: " << f.read_file("build/out.txt"));
                REQUIRE(f.read_file("build/out.txt") == "F=%f G=%<gen> build/gen.h\n");
            }
        }
    }
}

SCENARIO("A glob match containing a percent flag expands once, not twice", "[e2e][build][glob]")
{
    GIVEN("a foreach rule whose glob match contains a percent flag")
    {
        auto f = E2EFixture { "glob_mixed_space" };
        f.write_file("a%1fb.pct", "x\n");
        f.write_file("Tupfile", ": foreach *.pct |> echo GLOB=%g > %o |> %B.txt\n");
        f.mkdir("build");
        REQUIRE(f.pup({ "configure", "-B", "build" }).success());
        ...
        WHEN("the rule runs")
        {
            ...
            THEN("the command sees the match itself, not the match re-scanned for flags")
            {
                REQUIRE(f.exists("build/a%1fb.txt"));
                INFO("a%1fb.txt: " << f.read_file("build/a%1fb.txt"));
                REQUIRE(f.read_file("build/a%1fb.txt") == "GLOB=a%1fb\n");
            }
        }
    }
}

SCENARIO("A dependency scan resolves its parent's group reference", "[e2e][build][depscan][groups]")
{
    GIVEN("a compile rule whose command names a group")
    {
        auto f = E2EFixture { "glob_mixed_space" };
        f.write_file("a.c", "int main(void){return 0;}\n");
        f.write_file(
            "Tupfile",
            ": |> echo '#define X 1' > %o |> g%%f.h <gen>\n"
            ": a.c | <gen> |> gcc -c %f -o %o -DX=%<gen> |> a.o\n"
        );
        REQUIRE(f.build().success());

        WHEN("the recorded commands are listed")
        {
            auto result = f.pup({ "show", "index" });
            REQUIRE(result.success());

            THEN("the scan carries the member path whole, neither the marker nor a rescan of it")
            {
                INFO("stdout: " << result.stdout_output);
                REQUIRE(result.stdout_output.find("gcc -M") != std::string::npos);
                REQUIRE(result.stdout_output.find("%<gen>") == std::string::npos);
                REQUIRE(result.stdout_output.find("-DX=g%f.h") != std::string::npos);
                REQUIRE(result.stdout_output.find("ga.c.h") == std::string::npos);
            }
        }
    }
}
...
SCENARIO("A dep scan sees the same directory flag its compile sees", "[e2e][percent]")
{
    GIVEN("a root rule whose compile carries %d")
    {
        auto f = E2EFixture { "percent_d_scan" };
        f.write_file("a.c", "#include \"h.h\"\nint main(void) { return 0; }\n");
        f.write_file("h.h", "\n");
        f.write_file("Tupfile", R"(
: a.c |> gcc -DD=%d -c %f -o %o |> a.o
)");
        f.mkdir("build");
        f.write_file("build/tup.config", "");
        REQUIRE(f.pup({ "configure", "-B", "build" }).success());
        ...
        WHEN("the compile and its dep scan are compared")
        {
            auto const compdb = f.pup({ "show", "compdb", "-B", "build" });

            THEN("both spell the flag the same way")
            {
                REQUIRE(compdb.success());
                INFO("compdb: " << compdb.stdout_output);
                REQUIRE(compdb.stdout_output.find("\"-DD=.\"") == std::string::npos);
            }
        }
    }
}
