...
SCENARIO("A glob sees files generated by a directory parsed after it", "[e2e][glob][traversal]")
{
    GIVEN("a rule globbing files that a later-parsed sibling generates")
    {
        ...
        WHEN("built")
        {
            ...
            THEN("the build fails instead of shipping an empty match set")
            {
                INFO("stdout: " << result.stdout_output);
                INFO("stderr: " << result.stderr_output);
                REQUIRE_FALSE(result.success());
                auto output = result.stdout_output + result.stderr_output;
                REQUIRE(output.find("b/gen.q") != std::string::npos);
                REQUIRE(output.find("parsed later") != std::string::npos);
            }
        }
    }
}
...
SCENARIO("A foreach glob instantiates a command for a later-generated file", "[e2e][glob][traversal][foreach]")
{
    GIVEN("a foreach rule over files a later-parsed sibling generates")
    {
        ...
        WHEN("built")
        {
            ...
            THEN("the build fails instead of silently instantiating no command")
            {
                INFO("stdout: " << result.stdout_output);
                REQUIRE_FALSE(result.success());
                auto output = result.stdout_output + result.stderr_output;
...
SCENARIO("A glob whose producer is parsed first is not rejected", "[e2e][glob][traversal]")
{
