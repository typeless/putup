...
{
    auto arr = pup::IdArray64 {};

    SECTION("large values")
    {
        auto const big = std::uint64_t { 0xDEADBEEFCAFEBABE };
        arr.set(42, big);
        REQUIRE(arr.get(42) == big);
        REQUIRE(arr.contains(42));
    }

    SECTION("empty get returns 0")
    {
        REQUIRE(arr.get(0) == 0);
        REQUIRE_FALSE(arr.contains(0));
    }
    ...
    SECTION("for_each iterates occupied slots")
    {
        ...
        auto collected = std::vector<std::pair<std::uint32_t, std::uint64_t>> {};
        arr.for_each(
            [](std::uint32_t id, std::uint64_t value, void* ctx) {
                static_cast<std::vector<std::pair<std::uint32_t, std::uint64_t>>*>(ctx)->emplace_back(id, value);
            },
            &collected
        );

        REQUIRE(collected.size() == 2);
        REQUIRE(collected[0] == std::pair<std::uint32_t, std::uint64_t> { 5, 50 });
        REQUIRE(collected[1] == std::pair<std::uint32_t, std::uint64_t> { 10, 100 });
    }
