...
- reference: upstream aborts the update when a Tupfile fails to parse rather than skipping the directory and continuing — `process_create_nodes` in `updater.c` passes a hardcoded `keep_going = 0` to `execute_graph`, and `do_keep_going` reaches only the update call — so it has no state in which a run both refused a directory and succeeded. putup skips the directory under keep-going, which is what makes this sentence necessary; the deviation is in scope, not in status. The trigger names a build because `configure` sets `root_config_only`, so its contract is producing the root config rather than evaluating every Tupfile, and a refused directory need not impede it; `configure -k` returns zero on a refusal and no requirement here covers it — what a refused directory contributes to the work is keep-going's REQ-KEEPGOING-REFUSED-RUNS-NOTHING, which does bind configure
- discharge: test "Scenario: A keep-going build that refused a directory does not report it up to date"
- discharge: test "Scenario: keep-going build preserves outputs of a Tupfile that fails to parse"
- discharge: test "Scenario: keep-going build preserves outputs of a Tupfile that fails to evaluate"

While keep-going is in effect, when a directory's Tupfile fails to evaluate during a build, putup
shall exit nonzero.
