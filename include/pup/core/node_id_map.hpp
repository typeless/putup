...
public:
    ...
    [[nodiscard]]
    auto get(NodeId id) const -> std::uint32_t
    {
        auto idx = static_cast<std::uint32_t>(node_id::index(id));
    ...
    [[nodiscard]]
    auto contains(NodeId id) const -> bool
    {
        auto idx = static_cast<std::uint32_t>(node_id::index(id));
...
public:
    ...
    auto contains(NodeId id) const -> bool
    {
        return offsets_.contains(id);
    }

    auto resize_files(std::uint32_t n) -> void
    {
        offsets_.resize_files(n);
        lengths_.resize_files(n);
    }
    auto resize_commands(std::uint32_t n) -> void
    {
        offsets_.resize_commands(n);
        lengths_.resize_commands(n);
    }
    auto resize_conditions(std::uint32_t n) -> void
    {
        offsets_.resize_conditions(n);
        lengths_.resize_conditions(n);
    }
    auto resize_phis(std::uint32_t n) -> void
    {
        offsets_.resize_phis(n);
        lengths_.resize_phis(n);
    }
