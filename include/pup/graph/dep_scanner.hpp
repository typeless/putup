...
public:
    ...
    [[nodiscard]] virtual auto matches(CommandInfo const& cmd) const -> bool = 0;

    /// Check if command already has dependency generation enabled
    [[nodiscard]] virtual auto has_dep_flags(std::string const& cmd) const -> bool = 0;
    ...
    [[nodiscard]] virtual auto build_dep_command(CommandInfo const& cmd) const
        -> std::optional<std::string> = 0;

    /// Get the dependency extraction specification
    [[nodiscard]] virtual auto dep_spec() const -> DepSpec = 0;
...
[[nodiscard]] inline auto make_dep_display(std::vector<std::string> const& inputs) -> std::string
{
...
public:
    ...
    [[nodiscard]] auto find_match(CommandInfo const& cmd) const -> DepScanner const*;
    ...
    [[nodiscard]] auto empty() const -> bool { return scanners_.empty(); }
    [[nodiscard]] auto size() const -> std::size_t { return scanners_.size(); }
