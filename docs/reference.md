...
```
Error: Missing config file 'linux/tup.config' generated by build rules.
       Run 'pup configure' first.
```

This prevents silent failures where `@(VAR)` references would expand to empty strings due to missing config files.
...
```
Error: Missing config file 'sub/tup.config' generated by build rules.
       Run 'pup configure' first.
```

Cause: Your project has rules that generate `tup.config` files, but you ran `pup build` without first running `pup configure`.

Fix: Run `pup configure` to generate the config files, then run `pup build`:

```bash
pup configure    # Generate tup.config files
pup              # Build with generated configs
```

This two-pass workflow is needed when subprojects require per-directory config files generated from a central configuration. See Section 3.6 for details.
