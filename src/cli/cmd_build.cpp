...
) -> int
{
    ...
    // Check for missing config files that would be generated by build rules
    auto configs = find_config_commands(ctx.graph(), ctx.layout().source_root);
    for (auto const& cfg : configs) {
        if (!cfg.exists) {
            fmt::print(stderr, "Error: Missing config file '{}' generated by build rules.\n", cfg.output_path);
            fmt::print(stderr, "       Run 'pup configure' first.\n");
            return EXIT_FAILURE;
        }
    }
